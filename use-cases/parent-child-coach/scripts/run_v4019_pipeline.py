# @persistent — v4.0.23 生产管线测试执行器（背景物理隔离；升级 prompt 时随版本链同步）
"""v4.0.23 真实管线批量测试器 —— 快慢通道窗口 + 背景物理隔离 + Codex 裁判 + child_insight。
...
__version__ = "v4.0.23-e10"
# 自身迭代 changelog（e1→e2→…→e10，不随 prompt 版本跳号；e8 起前缀跟随 prompt 升 v4.0.22）：
# e10 — 背景物理隔离（配合 v4.0.23 prompt，用户 2026-08-08 定「独立LLM摘要 + fast/slow都隔离」）：
#       · 背景段不再喂原文，改用独立摘要模型（SUMMARY_MODEL，缺省星鸾 claude-opus-4-7，≠GEN_MODEL
#         满足铁律#6）压缩成语义概述——无原文可抄 → 背景泄漏物理归零（措辞级禁引已证到顶）
#       · summarize_background：独立模型把背景压成 ≤3 句概述，明令不得保留 ≥4 字原文连续片段
#       · 概述泄漏兜底：概述产物跑 detect_background_leak(overview, bg, "") 仍含 ≥6 字片段
#         → 降级极简占位「前文约 N 字，原文已省略」，保证物理零素材
#       · 背景标签改「仅作理解，已概述，无原文可引用」；fast/slow 两通道都隔离
#       · 概述结果 dict 缓存（同 background 不重复调用），相邻窗背景重叠大降成本
# e8 — prompt 升版 v4.0.22：执行器 PROMPT_VERSION v4.0.19→v4.0.22、__version__ 前缀同步改名
#       （v4.0.19-e7→v4.0.22-e8），PROMPT_MAP 新增 v4.0.22 生产映射；v4.0.19 保留为原版回退锚点
# e9 — 任务C 慢通道背景泄漏治理 + 任务B FC_BG_LEAK：
#       · 慢通道背景段标注「供理解，非生成依据」→「仅作理解，严禁引用其原句」（慢通道同禁引，
#         prompt 通用禁引规则自动覆盖，无需改规则文本）
#       · 慢通道也跑 detect_background_leak 硬门禁判定（原 else 硬编码 False 移除）
#       · 背景泄漏违规统一记 failure_codes=["FC_BG_LEAK"]（fast+slow），summary 加 fc_bg_leak 统计
#       · 测试智能体侧补机制：models.py FAILURE_CODE_LABELS + case_generator 对抗引导 + dashboard 列表
...
# e7 — 快速通道统一窗口机制（配合 v4.0.22 prompt + channel_spec 统一三类词）：
#       严重/警告/机会三类词统一「即刻分析」（移除 warning 等50字 general 分支）；
#       分析窗口前取 150 字、背景 900 字（含150 → 背景 750 + 窗口 150），<80 取消；
#       命中后 80 字保护窗口内新关键词不再扫描；slow 通道保持 lookback=500 不变；
#       feed 背景：fast 用 run_demo Popup.background（前900含150），slow 保持 TextBuffer lookback
PROMPT_VERSION = "v4.0.23"   # 适配的 prompt 版本（默认；可 --prompt 切换，见 PROMPT_MAP）
...
PROMPT_MAP = {
    ...
    "v4.0.19": PROJECT / "system_prompt_v4.0.19.txt",  # 原版回退锚点
    "v4.0.22": PROJECT / "system_prompt_v4.0.22.txt",  # 上一版（硬自检 · 对照锚点）
    "v4.0.23": PROJECT / "system_prompt_v4.0.23.txt",  # 生产版（背景物理隔离）
}
...
# ── 背景物理隔离：独立摘要模型（v4.0.23-e10，用户 2026-08-08 定）──
# 摘要模型必须 ≠ 生产生成模型 GEN_MODEL（铁律#6）。缺省星鸾 claude-opus-4-7
# （XINGLUAN 是 Anthropic 原生 API，用 litellm anthropic provider，见 auto_evolve/dual_client.py：
#   XINGLUAN_BASE_URL/AUTH_TOKEN + Anthropic client），真正独立于 deepseek 生成模型。
# 未配置 SUMMARY_API_BASE/KEY 时降级极简占位（不调 LLM，物理隔离仍成立）。
SUMMARY_MODEL = os.getenv("SUMMARY_MODEL", "anthropic/claude-opus-4-7")
SUMMARY_API_BASE = os.getenv("SUMMARY_API_BASE", os.getenv("XINGLUAN_BASE_URL", ""))
SUMMARY_API_KEY = os.getenv("SUMMARY_API_KEY", os.getenv("XINGLUAN_AUTH_TOKEN", ""))
SUMMARY_AVAILABLE = bool(SUMMARY_API_BASE and SUMMARY_API_KEY)


class _SummaryAdapter:
    """独立摘要适配器——把背景原文压缩成语义概述，接口对齐 _ProductionAdapter。

    与生产生成模型 GEN_MODEL 物理隔离（不同模型+不同端点），满足铁律#6。
    """

    def __init__(self, model=None, api_base=None, api_key=None):
        self.model = model or SUMMARY_MODEL
        self.api_base = api_base if api_base is not None else SUMMARY_API_BASE
        self.api_key = api_key if api_key is not None else SUMMARY_API_KEY

    def generate_with_chat_format(self, messages, temperature=0.3, max_tokens=200) -> str:
        resp = litellm.completion(
            model=self.model,
            messages=messages,
...
_SUMMARY_SYS = (
    "你是一个亲子对话背景压缩器。把给定的家长-孩子对话背景段压缩成不超过 3 句的语义概述。"
    "要求：只保留『人物、话题、情绪、关键转折』四要素；不得保留任何原文中的连续原句片段"
    "（任何≥4字的原文片段都禁止逐字照搬）；不得引用具体骂人词、威胁词、贴标签词；"
    "用你自己的话复述大意即可。"
)


def summarize_background(bg: str, adapter, cache: dict) -> str:
    """把背景原文压成语义概述；无原文可抄 → 背景泄漏物理归零。

    - 空背景 → 空串（上游 `if background.strip()` 分支已处理）。
    - 摘要模型不可用 → 降级极简占位「前文约 N 字，原文已省略」（物理隔离仍成立）。
    - 概述泄漏兜底：概述产物仍含背景 ≥6 字原文片段 → 降级极简占位，保证物理零素材。
    - 结果按 bg 内容缓存，相邻窗口背景重叠大 → 显著降低重复 LLM 调用。
    """
    key = hash(bg)
    if key in cache:
        return cache[key]

    if not bg or not SUMMARY_AVAILABLE:
        placeholder = f"前文约 {len(bg)} 字：家长与孩子就先前话题持续交流，具体原话已省略。"
        cache[key] = placeholder
        return placeholder

    try:
        overview = adapter.generate_with_chat_format(
            [
                {"role": "system", "content": _SUMMARY_SYS},
                {"role": "user", "content": f"请压缩以下亲子对话背景：\n\n{bg}"},
            ],
            temperature=0.3,
            max_tokens=200,
        )
        overview = (overview or "").strip()
        # 概述泄漏兜底：概述仍含背景 ≥6 字原文片段 → 降级极简占位
        leaked, _ = detect_background_leak(overview, bg, "", min_total=6)
        if leaked or not overview:
            overview = f"前文约 {len(bg)} 字：家长与孩子就先前话题持续交流，具体原话已省略。"
    except Exception:
        overview = f"前文约 {len(bg)} 字：家长与孩子就先前话题持续交流，具体原话已省略。"
...
def run_case(
    ...
    summary_adapter: Optional[_SummaryAdapter] = None,
    ...
    # 背景物理隔离：每 case 独立缓存（main 传入），跨 case 背景不串、防摘要内容跨题污染
    ...
    for p_idx, p in enumerate(popups):
        ...
        # 背景段来源区分通道（对齐 channel_spec 统一快通道窗口）：
        #   fast —— 用 run_demo Popup.background（触发点前 FAST_BACKGROUND=900 含分析窗口150
        #           → 前 750 字背景），与生产快通道窗口完全一致；p.background 缺失时兜底
        #           按 900-150=750 从 window_start 前推（同源不同算，逻辑等价）
        #   slow —— 保持生产 TextBuffer lookback=500 相对滑动不变（慢通道不改动）
        if p.channel == "fast":
            background = getattr(p, "background", "") or dialogue[
                max(0, p.window_start - 750):p.window_start
            ]
        else:
            background = dialogue[
        ...
        # ── 背景物理隔离（v4.0.23-e10）：背景段不喂原文，只喂独立 LLM 语义概述 ──
        # 无原文可抄 → 背景泄漏物理归零（措辞级禁引软/硬自检已证到顶，必须结构层解决）。
        # 两通道统一隔离；概述按 background 内容缓存，相邻窗口重叠大降重复调用。
        background_overview = summarize_background(background, summary_adapter, summary_cache)
        bg_label = "## 背景（仅作理解，已概述，无原文可引用）"
        dialogue_for_gen = (
            f"{bg_label}\n{background_overview}\n\n"
            f"## 本轮重点分析（弹窗生成依据）\n{p.context_window}"
            if background_overview.strip() else p.context_window
        )
        ...
        if use_real:
            ...
            if (
                ...
                entry = {
                    ...
                    "source": "production_popup_generator_v4.0.23",
        ...
        # ── 背景泄漏硬门禁（e6 快速通道；任务C 扩展到慢通道）──
        # fast + slow 统一判违规（慢通道背景段已改禁引标注），background_leak=True →
        # FC_BG_LEAK 失败代码；窗口=本轮 p.context_window，背景=本窗背景段。
        ...
        # FC_BG_LEAK 失败代码（任务B）：背景泄漏硬门禁违规（background_leak=True）→ 记入
        # 弹窗失败代码，供执行器报告/测试智能体 ingest 后驱动迭代者针对性修 prompt 禁引规则。
        entry["failure_codes"] = ["FC_BG_LEAK"] if entry.get("background_leak") else []
...
def main():
    ...
    parser.add_argument("--prompt", type=str, default="v4.0.23",
                        help="提示词版本: v4.0.12 / v4.0.18 / v4.0.19 / v4.0.22 / v4.0.23（默认 v4.0.23）")
    ...
    # 背景物理隔离：独立摘要适配器（SUMMARY_MODEL=星鸾 Opus，≠GEN_MODEL 满足铁律#6）。
    # 未配置 XINGLUAN 时降级极简占位（物理隔离仍成立，只是无 LLM 概述）。
    summary_adapter = _SummaryAdapter() if SUMMARY_AVAILABLE else None
    summary_cache: dict = {}
    print(f"背景概述模型: {SUMMARY_MODEL} {'✅' if SUMMARY_AVAILABLE else '⚠️ 未配置→降级极简占位'}")
    generator = PopupGenerator(
    ...
    for idx, case in enumerate(dataset):
        ...
        result = run_case(
            ...
            summary_adapter=summary_adapter, summary_cache=summary_cache,
        )
    ...
    output = {
        ...
        "summary": {
            ...
            # FC_BG_LEAK（任务B）：背景泄漏硬门禁违规弹窗总数（fast+slow）
            "fc_bg_leak": sum(
                1 for r in all_results
                for w in (r.get("fast_popups") or []) + (r.get("slow_popups") or [])
                if "FC_BG_LEAK" in (w.get("failure_codes") or [])
            ),
        },
        "results": all_results,
    }
