"""v1.7 vs v2.2.1 vs v2.3 · N=30 · GLM-5.2 Judge"""
...
# ── Judge prompt template (GLM-5.2, batch 3 versions) ──
JUDGE_HEADER = """你是亲子沟通弹窗评估专家。给三段弹窗从五个维度打分(1-5整数)，分别标为X、Y、Z。
...
### 五个维度
1. being_seen (1-5): 家长读完会不会心里轻轻动一下——"你懂我"？先接住了家长的发心和难处？
2. dialogue_fidelity (1-5): 每个判断都能在对话原文中找到确切依据？没有脑补？
3. core_insight (1-5): 是否抓住了这段对话里最该被看见的点？挠到痒处还是隔靴搔痒？
4. natural_language (1-5): 像真人在耳边说话？没有术语、没有框架标签、没有模板套话？
...
{"X":{"veto":null,"being_seen":1-5,"dialogue_fidelity":1-5,"core_insight":1-5,"natural_language":1-5,"warmth":1-5,"brief":"理由"},"Y":{...},"Z":{...}}
"""
...
def judge_three(dialogue, px, py, pz, labels):
    """Judge 3 popups in one GLM-5.2 call. labels maps X/Y/Z to version names."""
    # Shuffle to avoid position bias
    versions = list(zip(["X", "Y", "Z"], [px, py, pz], [labels[0], labels[1], labels[2]]))
    rng.shuffle(versions)

    popup_text = ""
    for key, popup, label in versions:
        popup_text += f"\n### {key}弹窗\n{popup}\n"

    prompt = JUDGE_HEADER + dialogue + popup_text + JUDGE_FOOTER
    ...
    for attempt in range(3):
        try:
            ...
            # Parse JSON from output (remove ANSI, extract JSON)
            clean = re.sub(r'\x1b\[[0-9;]*m', '', output)
            m = re.search(r'\{.*\}', clean, re.DOTALL)
            if m:
                parsed = json.loads(m.group(0))
                # Remap shuffled keys back to version names
                key_to_label = {key: label for key, _, label in versions}
                result_dict = {}
                for k in ["X", "Y", "Z"]:
                    if k in parsed:
                        result_dict[key_to_label[k]] = parsed[k]
                return result_dict
...
# ── Run ──
print(f"N=30 | seed=42 | Judge: baidu/glm-5.2")
...
for i, case in enumerate(cases):
    ...
    # Generate 3 versions in parallel (DeepSeek)
    ...
    # Judge all 3 in one GLM-5.2 call (blind, shuffled)
    judge_results = judge_three(dialogue, p17, p22, p23,
                                labels=["v1.7", "v2.2.1", "v2.3"])

    if not judge_results:
        print(f"[{i+1:2d}/30] {title[:25]:<25} | JUDGE FAILED", flush=True)
        continue

    s17r = judge_results.get("v1.7", {})
    s22r = judge_results.get("v2.2.1", {})
    s23r = judge_results.get("v2.3", {})
    ...
    veto_str = f"veto={'v17' if v17f else ''}{'v221' if v22f else ''}{'v23' if v23f else ''}"
    print(f"[{i+1:2d}/30] {title[:25]:<25} | v1.7={s17:.4f} v2.2.1={s22:.4f} v2.3={s23:.4f} | "
          f"1st={ranked[0][0]} {veto_str} | w={w17}/{w22}/{w23} | {eta}", flush=True)
...
# Avoid zero stdev for single sample
std17 = stdev(all_s17) if len(all_s17) > 1 else 0
std22 = stdev(all_s22) if len(all_s22) > 1 else 0
std23 = stdev(all_s23) if len(all_s23) > 1 else 0
...
out = {
    ...
    "summary": {
        "v17_mean": mean(all_s17), "v17_std": std17,
        "v221_mean": mean(all_s22), "v221_std": std22,
        "v23_mean": mean(all_s23), "v23_std": std23,
