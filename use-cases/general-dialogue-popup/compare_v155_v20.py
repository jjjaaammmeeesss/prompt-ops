"""对比 v1.5.5 vs v2.0 成人通用弹窗 — 10 个职场场景 × n=3。

生成：DeepSeek v4-pro（国产模型铁律）
评审：GLM 5.2（百度千帆，五维打分 + v2.0 硬规则检查）
维度：insight, third_party, language, evidence, focus（删除 suggestion，新增 third_party）
输出：results/compare_tests/compare_v155_v20_n{n}_{timestamp}.json

Usage:
    python compare_v155_v20.py
    python compare_v155_v20.py --n 3  # 每 case 跑 n 次（默认 3）
"""
...
def main(n_runs: int = 3):
    ...
    report = {
        ...
        "summaries": [v155_summary, v20_summary],
        "v1.5.5": v155_results,
        "v2.0": v20_results,
    }

    out_path = RESULTS_DIR / f"compare_v155_v20_n{n_runs}_{timestamp}.json"
    out_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info("报告已保存: %s", out_path)

    # ── 打印汇总 ──
    print("\n" + "=" * 60)
    print("  v1.5.5 vs v2.0 对比结果")
    print("=" * 60)
    ...
    print()
    for s in [v155_summary, v20_summary]:
        print(f"  ── {s['label']} ──")
    ...
    diff = v20_agg - v155_agg
    winner = "v2.0" if diff > 0 else "v1.5.5" if diff < 0 else "平手"
    print(f"  Δ aggregate: {diff:+.3f} → {winner} 领先")
    print()
    # 关键维度对比
    for d in ["third_party", "evidence", "focus"]:
        d_diff = round(v20_summary["soft_means"][d] - v155_summary["soft_means"][d], 3)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="对比 v1.5.5 vs v2.0")
