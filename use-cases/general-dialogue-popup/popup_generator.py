# @persistent — 一般场景弹窗的可复用生成器（预分析解析）
"""一般场景弹窗生成器：解析 LLM 输出的预分析块（`==========` 为界），剥离元信息，返回纯弹窗正文。

适配 system_prompt_v2.0.txt 的预分析流程（元信息 / 关键句归属 / 错别字，无类型项）。

从亲子 realtime/popup_generator.py 移植通用剥离逻辑，裁剪掉亲子特定内容：
- 不做类型三选一（诊断式/鼓励式/看见孩子）解析
- 不做 ta 人称、卦象 tone、P2 话术、FC_TONE_OFF/FC_CHILD_INSIGHT 等
仅保留：`==========` partition + 按行剥离预分析元信息行（兜底防泄露）+ 前缀清理 + 安静判定。
...
def parse_popup_output(text: str) -> str | None:
    ...
    安静信号（v2.0 规则）：输出"安好"两个字时表示决定不弹窗。
    """
    if not text:
        return None
    text = text.strip()
