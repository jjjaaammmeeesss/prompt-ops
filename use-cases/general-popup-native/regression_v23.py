"""ty2.4 回归验证 — 5 个已知样本（ws08/ws09 专家打标窗口）。

验证 ty2.4 的核心改动是否生效：
  - 输出不含"你"
  - 字数 60–140
  - 行业惯例豁免（ws08丁 → 应安静）
  - 自然收束（ws08戊、ws09丁 → 应安静）
  - 框架转换检测（ws09甲 → 应弹）
  - 决策权边界识别（ws09乙 → 应弹）

Usage:
    python regression_v23.py
"""
...
logger = logging.getLogger("regression_v23")
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("ty2.4 回归验证 — %d 个样本", len(REGRESSION_CASES))
    logger.info("生成模型: %s", GEN_MODEL)
    logger.info("=" * 50)
    ...
    for case in REGRESSION_CASES:
        ...
        if expect == "安静":
            if popup_text is None:
                ...
                ni_violation = '你' in popup_text
                over_words = len(popup_text) > 140
                flags = []
                if ni_violation: flags.append("含'你'")
                if over_words: flags.append(f"字数{len(popup_text)}>140")
                results.append({
            ...
            if popup_text is None:
                ...
                wc = len(popup_text)
                has_ni = '你' in popup_text
                over_140 = wc > 140
                over_180 = wc > 180

                violations = []
                if has_ni:
                    violations.append("含'你'字")
                if over_180:
                    violations.append(f"字数{wc}>180（死线）")
                elif over_140:
                    violations.append(f"字数{wc}>140（目标线）")
                ...
                logger.info("  字数: %d, 含'你': %s, 关键词: %s/%s",
                            wc, has_ni, len(kw_hits), len(keywords))

                # 通过条件：无"你" + 字数≤180 + 有关键词命中（或字数合规）
                passed_case = (not has_ni) and (not over_180)
                if not passed_case:
                    logger.warning("  ❌ FAIL — %s", ", ".join(violations))
                    failed += 1
                else:
                    if violations:
                        logger.info("  ⚠ WARN — %s（不阻断）", ", ".join(violations))
                    logger.info("  ✅ PASS")
                    passed += 1
    ...
    out_path = RESULTS_DIR / f"regression_v23_{timestamp}.json"
    ...
    for r in results:
        ...
        extra = ""
        if r.get("violations"):
            extra = f"  [{', '.join(r['violations'])}]"
        if r.get("wc"):
            extra += f"  [{r['wc']}字]"
        print(f"  {status} {r['case_id']}{extra}")
